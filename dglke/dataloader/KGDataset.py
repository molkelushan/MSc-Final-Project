...
class KGDatasetOBL2021(KGDataset):

    def __init__(self, path, name='OBL2021'):
        self.name = name
        ...
        dl = OBL2021Dataset(self.path)

        self.delimiter = '\t'
        self.entity2id, self.n_entities = self.read_entity(os.path.join(self.path, 'entities.tsv'))
        self.relation2id, self.n_relations = self.read_relation(os.path.join(self.path, 'relations.tsv'))
        self.train = self.read_triple(os.path.join(self.path, 'train.tsv'), "train")
        self.valid = self.read_triple(os.path.join(self.path, 'valid.tsv'), "valid")
        self.test = self.read_triple(os.path.join(self.path, 'test.tsv'), "test")
...
def get_dataset(data_path, data_name, format_str, delimiter='\t', files=None, has_edge_importance=False):
    if format_str == 'built_in':
        ...
        elif data_name == 'OBL2021':
            dataset = KGDatasetOBL2021(data_path)
        else:
